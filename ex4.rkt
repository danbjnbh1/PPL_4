  ...
  (lambda (tree1 tree2 succ fail)
    (if (and (empty? tree1) (empty? tree2))
      ...
      (if (or (empty? tree1) (empty? tree2))
        ...
        ; two non empty trees
        (if (and (not (list? (car tree1))) (not (list? (car tree2))))
          ; two leafs
          (equal-trees$ 
            (cdr tree1)
            (cdr tree2) 
            (lambda (res)
                (append$ (list (cons (car tree1) (car tree2))) res succ))
            fail
          )
          (if (and (list? (car tree1)) (list? (car tree2)))
            ; two sub trees
            (equal-trees$
              (car tree1)
              (car tree2)
              (lambda (res-car) 
                (
                  ...
                  (cdr tree2)
                  (lambda (res-cdr)
                     (append$ (list res-car) res-cdr succ))
                  fail))
              fail
            )
            ; one subtree and one leaf
